...
func (k *kBroker) Publish(topic string, msg *broker.Message, opts ...broker.PublishOption) error {
	headers := make([]sarama.RecordHeader, 0, len(msg.Header))
...
func (k *kBroker) Subscribe(topic string, handler broker.Handler, opts ...broker.SubscribeOption) (broker.Subscriber, error) {
	...
	go func() {
		for {
			...
			case err := <-cg.Errors():
				if err != nil {
					k.opts.Logger.Logf(logger.ErrorLevel, "consumer error:", err)
				}
			default:
				...
				case nil:
					continue
				default:
					k.opts.Logger.Log(logger.ErrorLevel, err)
				}
			}
		}
	}()
...
func (h *consumerGroupHandler) ConsumeClaim(sess sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	for msg := range claim.Messages() {
		...
		if err == nil && h.subopts.AutoAck {
			...
			p.err = err
			h.kopts.Logger.Logf(logger.ErrorLevel, "[kafka]: subscriber error: %v", err)
		}
	}
	return nil
}
